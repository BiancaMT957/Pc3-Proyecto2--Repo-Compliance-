...
@pytest.mark.parametrize(
    "gitignore, env_in_gitignore, expected_status, expected_sev",
    [
...
def test_check_env_in_gitignore(
    repo_factory, gitignore, env_in_gitignore, expected_status, expected_sev
):
...
@pytest.mark.parametrize(
    "targets, expected_status, missing_subset",
    [
...
@pytest.mark.parametrize(
    "dot_env, settings_secret, config_secret, expected_status, expected_sev",
    [
...
def test_check_config_via_env(
    repo_factory, dot_env, settings_secret, config_secret, expected_status, expected_sev
):
    repo = repo_factory(
        dot_env=dot_env,
...
def test_check_config_via_env_ok_with_example(repo_factory):
    ...
    assert res["severity"] in {
        "LOW",
        "MEDIUM",
    }  # implementacion permite bajar a LOW con .env.example
